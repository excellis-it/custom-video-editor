...
class VideoEditor {
  constructor() {
    ...
    this.thumbnailRemovalTime = parseInt(this.videoContainer.dataset.thumbnailTiming) || 10; // dynamic seconds from data attribute
  ...
  checkThumbnailRemoval() {
    ...
    if (elapsed < this.thumbnailRemovalTime) {
      // Show thumbnail if within the first 10 seconds
      if (!this.thumbnailShown) {
        this.thumbnailOverlay.classList.remove("hidden");
        this.thumbnailShown = true;
      }
    } else {
      // Hide thumbnail if past 10 seconds
      if (this.thumbnailShown) {
