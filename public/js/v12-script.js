...
class VideoEditor {
    ...
    async init() {
        ...
        if (ccToggle) ccToggle.checked = true;
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll(".subtitle-option").forEach((btn) => {
            btn.addEventListener("click", (e) => {
                ...
                if (ccToggle) {
                    ccToggle.checked = (lang === 'en' || lang === 'original');
                }
        ...
        if (ccToggle) {
            ccToggle.addEventListener('change', () => {
                if (!this.player || !this.player.setCaptions) return;

                if (ccToggle.checked) {
                    // Enable subtitles
                    this.player.setCaptions(this.currentLang || 'original');
                    this.subtitlesEnabled = true;
                    console.log('CC Enabled');
                } else {
                    // Disable subtitles
                    this.player.setCaptions('off');
                    this.subtitlesEnabled = false;
                    this.subtitleOverlay.innerHTML = '';
    ...
    showSkipFeedback(seconds) {
        ...
        feedback.style.cssText = `position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 3rem; font-weight: bold; color: white; text-shadow: 0 4px 8px rgba(0,0,0,0.5); z-index: 100; animation: fadeOut 0.5s ease forwards;`;
